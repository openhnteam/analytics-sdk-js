...
const FIREBIRD_PREFIX = 'fire-bird'
...
async function modify(targetVersion) {
  ...
  for (const target of beModifiedPackages) {
    await modifyFireBirdVersion(target, targetVersion)
  }
}

async function modifyFireBirdVersion(pkgName, version) {
  const pkgRoot = getPkgRoot(pkgName)
  const pkgPath = path.resolve(pkgRoot, 'package.json')
  const pkg = require(pkgPath)
  const oldVersion = pkg.version
  if (pkg.name.startsWith(FIREBIRD_PREFIX)) {
    pkg.version = version
  }
  const dependencies = pkg.dependencies || {}
  Object.entries(dependencies).forEach(([dependent, dependentVersion]) => {
    // 拼接：前缀 + 当前包名 如果当前依赖中有被改的包，也把version修改
    const isExist = beModifiedPackages.some((pkg) => `${FIREBIRD_PREFIX}/${pkg}` === dependent)
    if (isExist) {
